...
def generate_response(prompt):
    ...
    position_ids = torch.arange(0, inputs['input_ids'].size(1), device=model.device).unsqueeze(0)
    ...
    try:
        ...
        print("position_ids shape:", inputs['position_ids'].shape)
        return response
    except Exception as e:
...
def rag_image_query():
    ...
    if image_search_results and "metadatas" in image_search_results and image_search_results["metadatas"][0]:
        ...
        if top_result_metadata and "caption" in top_result_metadata:
            ...
            return "No caption found for the retrieved image."
    else:
        return "No relevant image context found."

def main():
    print("Welcome to the Image Search and RAG service!")
    print("1. Image-to-Image Search")
    print("2. RAG Image Query")

    choice = input("Enter your choice (1 or 2): ")

    if choice == "1":
        image_path = input("Enter the path to your query image: \n")
        results = image_to_image_search(image_path, results=1)
