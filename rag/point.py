...
def find_best_match(query_image_path, retrieved_image_embedding, embedding_function, patch_size=(32, 32)):
    ...
    # Return the top matching patches (coordinates)
    top_matches = patch_similarities[:5]  # Adjust the number of top matches as needed
    return top_matches
def rag_image_query():
    ...
    while True:
        ...
        if option == "1":
            ...
            top_matches = find_best_match(query_image_path, reference_embedding, embedding_function)

            # Display results
            print("\nTop matching coordinates (x1, y1, x2, y2) and similarity score:")
            img_cv = cv2.imread(query_image_path)
            for (similarity, (x1, y1)) in top_matches:
                x2, y2 = x1 + 32, y1 + 32  # assuming chunk size of 32x32
                print(f"Coordinates: {(x1, y1, x2, y2)}, Similarity: {similarity}")
                cv2.rectangle(img_cv, (x1, y1), (x2, y2), (0, 255, 0), 2)

            # Save the image instead of displaying
            output_path = "/pic/chunk_comparison_result.jpg"
            cv2.imwrite(output_path, img_cv)
            print(f"Chunk comparison image saved as {output_path}")

        elif option == "2":
            image_path = input("Enter the path to your query image: ").strip()
            matched_img_id = image_to_image_search(image_path, results=1)
            if matched_img_id:
                print(f"Reference Image Retrieved: {matched_img_id}")
            else:
